...
import com.example.backend.constants.StatusConstants.ISSUE_STATUS_IN_PROGRESS
import com.example.backend.constants.StatusConstants.ISSUE_STATUS_OPEN
import com.example.backend.constants.StatusConstants.ISSUE_STATUS_RESOLVED
...
interface IssueRepository : JpaRepository<Issue, Long> {
  ...
  @Query("SELECT COUNT(i) FROM Issue i WHERE i.status = '$ISSUE_STATUS_OPEN' AND i.declarationDate BETWEEN :startDate AND :endDate")
  fun countOpenIssuesBetweenDates(startDate: LocalDateTime?, endDate: LocalDateTime?): Int

  @Query("SELECT COUNT(i) FROM Issue i WHERE i.status = '$ISSUE_STATUS_RESOLVED' AND i.declarationDate BETWEEN :startDate AND :endDate")
  fun countResolvedIssuesBetweenDates(startDate: LocalDateTime?, endDate: LocalDateTime?): Int

  @Query("SELECT COUNT(i) FROM Issue i WHERE i.status = '$ISSUE_STATUS_IN_PROGRESS' AND i.declarationDate BETWEEN :startDate AND :endDate")
